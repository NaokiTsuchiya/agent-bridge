 ...
 * see that it does is what a swap costs: one binding, no change to the interface, the ports or the
 * front end, and a deployment that still ships the resident runner.
 */
final class RunnerSubstitutionTest extends TestCase
{
    ...
    /**
     * The swap is one binding.
     *
     * Read off the source because that is the claim: a module that had to re-declare the resolver,
     * the settings or the front end would mean the execution layer is not the seam it is meant to be.
     *
     * @throws ReflectionException
     */
    #[Test]
    public function theSwapCostsOneBinding(): void
    {
        ...
        self::assertSame(1, substr_count($source, needle: '$this->bind('));
    }
}
