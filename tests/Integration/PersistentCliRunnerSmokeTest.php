...
/**
 * One round trip against the real binary, which is all this group carries.
 *
 * The fake reproduces the wire protocol, and issue #5's contract tests are what keep the two
 * honest with each other; what is left for this group is the one thing a stand-in cannot show —
 * that the runner drives an actual `claude` from an unknown thread to a finished turn.
 *
 ...
 */
#[Group('integration')]
final class PersistentCliRunnerSmokeTest extends TestCase
{
    ...
    {
        $cwd = realpath(TempDir::make('smoke-cwd'));
