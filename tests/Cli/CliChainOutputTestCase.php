 ...
 *
 * Which execution layer produces the pieces is the subclass's to say: streaming is a promise the
 * front end makes, and both runners have to keep it.
 *
 * @internal
 */
abstract class CliChainOutputTestCase extends TestCase
{
    ...
    /**
     * @param WorkingDirectoryResolver $directories where the thread's work happens
     *
     * @return AgentRunner the execution layer under test, pointed at the fake binary
     */
    abstract protected function runner(WorkingDirectoryResolver $directories): AgentRunner;
    ...
    private function answer(string $text, mixed $reply, mixed $status): CompletedTurn
    {
        ...
        $runner = $this->runner(new WorktreeWorkingDirectory($worktrees));
