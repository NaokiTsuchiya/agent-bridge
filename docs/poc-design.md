...
#### 4.4.4 SpawnCliRunner

ゴール 4 の 2 実装目 (issue #12)。**1 ターン = 1 プロセス**で、プロセスもマップもプールも持たない。
...
claude --print
  ...
  --allowedTools <許可リスト>
  --resume <derived> | --session-id <derived>
  <プロンプト>                     # ★ 位置引数。--input-format は付けない
```

- **stdin は起動直後に閉じる。** 閉じないと実 CLI が 3 秒待って警告を出す (4.4.2 の「ハマりどころ」)
- プロンプトが位置引数なので、`-` で始まる本文は実 CLI にフラグと読まれうる。one-shot 起動で実 CLI が何を受け付けるかの裏付けは契約テストの「末尾の位置引数」だけなので、`--` セパレータは入れていない (既知の限界)
- `close()` は保持するものが無いので何もしない
- cwd の解決経路も `--allowedTools` も実行バイナリも、常駐と同じコラボレータ / 設定から受け取る
- ターンの直列化は常駐と同じ (`ThreadId` ごとの mutex)。異なるスレッドは並行する

**セッション解決は常駐と同じ案 A′ だが、判定材料が 1 つ少ない。** 常駐は「`--resume` の 1 ターン目が出力なしで失敗した」ときに**プロセスが生き残っているか**でセッション消失と単なるターン失敗を区別する。都度起動ではプロセスが必ず終了するので、この区別ができない。したがって:
...
実装したフェイクのインタフェース (バイナリのパス、環境変数、シナリオファイルの形式、記録ファイルの形式、契約テストの回し方) は [`fake-claude-cli.md`](fake-claude-cli.md) にある。
