...
ツールの開始・完了は**パイプラインが引用行に包んでから** `StreamHandle` へ流す (`AnsweringTurn::TOOL_NOTICE`)。ポートが約束するのは「差分を append する」ことだけなので、ツール通知もテキストとしてここへ届く。アダプタはその**行の形**を手がかりに (`> ` で始まる行 1 本) 本文と切り分け、Slack が用意している置き場 — `task_update` チャンク — へ回す (`Slack\StreamChunks`)。
